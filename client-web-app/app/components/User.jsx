...
class User extends React.Component {
  ...
  render() {
    const selectors = ['info', 'chatting', 'heart'].map((selector, index) => {
      ...
      return (
        <img key={index} src={source} className="selector" onClick={() => this.changeSelector(index)} /> //eslint-disable-line
      );
    });
    ...
    const familiarLanguages = this.famLangArr.map(lang => (
      <MenuItem
        value={lang.name}
    ...
    const languageLevels = () => {
      this.famLangArr.forEach(function (lang) { //eslint-disable-line
    ...
    //   for (const lang of this.famLangArr) {
    //     if (this.state.famLanguage === lang.name) {
    //       return (
    //         <LanguageLevel
    //           value={lang.level}
    ...
    const interests = [{ icon: 'airplane', name: 'Travelling' }, { icon: 'books', name: 'Reading' }, { icon: 'diamond', name: 'Fashion' }, { icon: 'gamepad', name: 'Video Games' }].map((hobby) => {
      const source = `./png/${hobby.icon}.png`;
      return (
        <div className="interestsListWrap">
